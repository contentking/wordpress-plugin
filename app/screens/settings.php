...
?>
	...
	<table class = "contentking-setting">
		<tr>
			<th scope="row"> <?php echo esc_html__( 'API token', 'contentking-plugin' ); ?> </th>
...
if ( 5 < strlen( $contentking_client_token ) && '0' === $flag ) :
	...
	<p class="error">
	<?php esc_html__( 'It looks like your API token is not validated. Click to validate.','contentking-plugin' ); ?>
	</p>
	</form>
...
<ol>
	...
	<?php
	_e( // WPCS: XSS OK.
		'Copy the API token from the <strong>Integrations</strong> section.','contentking-plugin'
	);
			?>
		 </li>
	<li>
	<?php
	_e( // WPCS: XSS OK.
		'Come back to this WordPress screen and paste the token into the <strong>API token</strong> field.','contentking-plugin'
	);
			?>
		 </li>
	<li>
